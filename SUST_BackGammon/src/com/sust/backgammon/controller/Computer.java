...
public class Computer extends Player {
	...
	public Computer(Integer pn, Game game) {
		super(pn, "Computer", game);
		emm = new Expectiminimax();
	}

	public void play() {
		GameState init = new GameState(this.getGame().getCurrentBoard(), this
				.getGame().getDice());
		GameState res = emm.chooseMove(init);
		if (res == null) // no moves to play
		{
			this.getGame().getDice().useAllDices();
			return;
		}
		calcMoves(this.getGame().getCurrentBoard().numOfCheckInPipe, res.board,
				this.getGame().getDice());
		this.getGame().getDice().useAllDices();
		System.out.println(res); // print the result of the algorithm in the
									// console
	}
	...
	private void calcMoves(int[] init, int[] dest, Dice dice) {
		...
		if (!to.isEmpty()) {
			for (int i : to) {
				...
				{
					this.getGame().getBoardAnimation().getCurrentBoard()
							.sendToBar(i);
				}
	...
	private static void calcFromTo(int[] init, int[] dest,
			ArrayList<Integer> diff, ArrayList<Integer> to) {
		int counter = 0;
		for (int i = 0; i < 24; i++) {
			if (init[i] < dest[i]) {
	...
	public void calcMoves(Dice dice, ArrayList<Integer> from,
			ArrayList<Integer> to, int[] dest) {
			...
			if (!dice.isRollDouble()) {
				...
				if (to.contains(f + minDie) && dest[f + minDie] != 0) {
					...
					this.getGame().getBoardAnimation()
							.animateMove(f == -1 ? 25 : f, f + minDie);
					from.remove(Integer.valueOf(temp.get(i)));
				} else if (to.contains(f + (2 * minDie))
						&& dest[f + 2 * minDie] != 0) {
					to.remove(Integer.valueOf(f + (2 * minDie)));
					this.getGame().getBoardAnimation()
							.animateMove(f == -1 ? 25 : f, f + (2 * minDie));
					from.remove(Integer.valueOf(temp.get(i)));
				} else if (to.contains(f + (3 * minDie))
						&& dest[f + 3 * minDie] != 0) {
					to.remove(Integer.valueOf(f + (3 * minDie)));
					this.getGame().getBoardAnimation()
							.animateMove(f == -1 ? 25 : f, f + (3 * minDie));
					from.remove(Integer.valueOf(temp.get(i)));
				} else if (to.contains(f + (4 * minDie))) {
					to.remove(Integer.valueOf(f + (4 * minDie)));
					this.getGame().getBoardAnimation()
							.animateMove(f == -1 ? 25 : f, f + (4 * minDie));
					from.remove(Integer.valueOf(temp.get(i)));
				}
			}
		}
