...
public class GameState {
	...
	public double evaluate(int player) { // Huristic
		double eval = 0;
		...
		if (player == 1) {
			eval = (-0.025 * (pipeContainingMoreThanOneWhiteChecker - pipeContainingMoreThanOneRedChecker))
					- (0.02525 * (Math.abs(board[this.gyard(2)]) - board[this
							.gyard(1)]))
					+ 0.4
					* totalValueOfWhiteCheckers
			...
			eval = (0.025 * (pipeContainingMoreThanOneRedChecker - pipeContainingMoreThanOneWhiteChecker))
					...
					- 0.4 * totalValueOfRedCheckers 
					- 0.0125 * distanceForRedCheckers;
...
//					- 0.0125 * (distanceForRedCheckers - distanceForWhiteChekers);
	...
	public ArrayList<GameState> getChildren(int player) {
		...
		if (!this.hasGyard(this.board, player)) {
			...
			} else // cannot find children,will check for partial moves only
			{
				if (diceLeft == 2) {
					diceLeft--;
					this.getChildren(children, player, 0, diceLeft - 1,
							this.board);
					this.setDice(dice[1], dice[0]);
					this.getChildren(children, player, 0, diceLeft - 1,
							this.board);
			...
			if (diceLeft == 4) // doubles
			{
				...
				while (this.hasGyard(noGy[0], player) && diceLeft != 0) {
					moveLoc = this.countFrom(player)
							+ (op * this.dice[--diceLeft]);
	 ...
	 */
	public void getChildren(ArrayList<GameState> children, int player,
			int startLoc, int diceLeft, int[] board) {
		...
		for (int i = startLoc; i < 24; i++) {
			...
			if (this.playerHas(player, board[i])) {
				...
				if (this.canGather(player, tempBoard, i, diceLeft)) {
					...
					this.getChildren(children, player, i, diceLeft - 1,
							tempBoard);
